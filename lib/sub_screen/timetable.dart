        ...
        child: Stack(
          ...
          children: [
            Align(
              ...
              child: global.textWidget("${global.accObj!.department!.toUpperCase()}  [${global.accObj!.year!.toUpperCase()}]  - ${global.accObj!.section!.toUpperCase()} | ${DateFormat('EEEE').format(DateTime.now())}")
            ),
//
//
            ShaderMask(
                    ...
                    TimelineTile(
                      ...
                      contents: SizedBox(
                        ...
                        child: Align(
                          ...
                          child: global.textWidget("College commencement")
                        ),
                      ),
                      node: TimelineNode(
                        position: 0.25,
                        indicator: OutlinedDotIndicator(borderWidth: 1,),
                        startConnector: null,
                        endConnector: dlc,
                      ),
                    ),
TimelineTile(
                      ...
                      contents: SizedBox(
                        ...
                        child: Align(
                          alignment: Alignment.centerLeft,
                          child: global.textWidget("HS3251 - English ?")
                        ),
                      ),
                      node: TimelineNode(
                        position: 0.25,
...
TimelineTile(
                    ...
                    TimelineTile(
                      ...
                      contents: SizedBox(
                        ...
                        child: Align(
                          alignment: Alignment.centerLeft,
                          child: global.textWidget("College cessation")
                        ),
                      ),
                      node: TimelineNode(
                        position: 0.25,
...
List<Widget> createTTE() {
  ...
  Widget widget (String timeStr, String contentStr, {bool coloredGray = false, bool startC = true, bool endC = true}) => TimelineTile(
                      ...
                      node: TimelineNode(
                        position: 0.25,
                        indicator: OutlinedDotIndicator(borderWidth: 1,color: coloredGray != false ? Colors.grey : null,),
                        startConnector: startC==true ? (coloredGray != false ? dlcGray : dlc) : null,
                        endConnector: endC==true ? (coloredGray != false ? dlcGray : dlc) : null,
                      ),
                    );
  ...
  if(ttT.isEmpty == true || ttS.isEmpty == true) {
    ...
    l.add(widget("??", "Time table information is not available or not provided yet."));

  } else if(ttS[timeNow.weekday.toString()] == null) {

    l.add(widget("00:00", "Time table data is not provided for this specific day."));
    ...
    l.add(widget(ttT.first, todayTTS.first, startC: false, coloredGray: timeNow.isAfter(DateFormat("hh:mm").parse(ttT[0]))));
